...
class Ratio(object): 
	def __init__(self, aug_per=1, rand_cut=0.3):
...
class Ratio_Bidirection(object): 
	def __init__(self, aug_per=3, rand_cut=None, up_range=[-0.2, 0.2], bottom_range=[-0.3, 0.7]):
	...
	def get_ratio(self, boxlist, is_train):
		...
		for target in boxlist:
			...
			for k in range(n):
				...
				if is_train:
					if pad == 1.0:
						...
						if not is_train:
							pad = 0.
					elif pad != 1.0 and 1.-pad-uppad > 0:
						...
						p_bbox_repeat = p_bbox.repeat(self.aug_per, 1)

						curr_h = p_bbox[3] - p_bbox[1]
						curr_w = p_bbox[2] - p_bbox[0]
						
						# jittering upside down 
						for j in range(self.aug_per):
						
							if j < self.aug_per - 2:
								rand_bottom = random.uniform(self.bottom_range[0], self.bottom_range[1])
								p_bbox_repeat[j, 3] -= curr_h * rand_bottom
								sample_uppad.append(0.)
								sample_pad.append(rand_bottom)

							elif j == self.aug_per - 1:
								rand_up = random.uniform(self.up_range[0], self.up_range[1])
								p_bbox_repeat[j, 1] += curr_h * rand_up
								sample_uppad.append(rand_up)
								sample_pad.append(0.)
						...
						new_bbox.extend(p_bbox_repeat.tolist())
						new_pad.extend(sample_pad)
						new_uppad.extend(sample_uppad)
			...
			new_uppad = torch.tensor(new_uppad).unsqueeze(-1)
			new_pad = torch.tensor(new_pad).unsqueeze(-1)
			
			new_bboxlist.add_field("pad_ratio", torch.cat([new_uppad, new_pad], dim=1))
			return_boxlist.append(new_bboxlist)

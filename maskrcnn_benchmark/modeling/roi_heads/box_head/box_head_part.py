...
from .completeness_part import Ratio, Ratio_Bidirection

from maskrcnn_benchmark.layers import OIMLoss
from maskrcnn_benchmark.layers import OIMLoss_Part, OIMLoss_PartBidirection
from maskrcnn_benchmark.layers import est_decode_regval2proposal, est_decode_bidirection_regval2proposal
...
class ROIBoxHead_part(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        num_part = cfg.MODEL.APNET.NUM_PARTS

        use_padreg_bidirection = cfg.MODEL.APNET.USE_PADREG_BIDIRECTION
        self.regval2proposal = est_decode_bidirection_regval2proposal if use_padreg_bidirection else est_decode_regval2proposal
        ratio_est = Ratio_Bidirection if use_padreg_bidirection else Ratio
        self.ratio_estimator = ratio_est(cfg.MODEL.APNET.DISCARD_NOKP)
        
        oim_part = OIMLoss_PartBidirection if use_padreg_bidirection else OIMLoss_Part
        self.oimloss = nn.ModuleList()
        for j in range(num_part):
            self.oimloss.append(oim_part(cfg.MODEL.REID_CONFIG.FEAT_DIM, cfg.MODEL.REID_CONFIG.NUM_IDS))
        
        if cfg.MODEL.APNET.RSFE:
            self.oimloss.append(OIMLoss(cfg.MODEL.REID_CONFIG.FEAT_DIM, cfg.MODEL.REID_CONFIG.NUM_IDS))
        ...
        self.query_by_gt = cfg.MODEL.APNET.PADREG_QUERY_GT
    ...
    def forward(self, features_, proposals, targets=None, query=False):
        ...
        if self.training:
            ...
            """
            return_bboxlist = self.regval2proposal(proposals, add_padratio=False)
